...
const HOST_SSH_AUDIT_CATEGORY     = 'host_ssh_audit';
/** Suppress repeat host-SSH-audit alerts (per severity) within this window. */
const HOST_SSH_AUDIT_DEDUP_WINDOW_MS = 3 * 60 * 60 * 1000;
...
async function runCredentialAuditTask() {
  ...
  // `alerts.session_id` is a FOREIGN KEY into sessions(session_id). The ips
  // alertRef is NOT a session id, so passing it here violated the FK (with
  // foreign_keys=ON) and threw, silently dropping every credential_audit alert
  // since the task was refactored off the orchestrator. Persist null like every
  // other no-session cron task and keep the ref in the body for traceability.
  createAlert({
    session_id: null,
    severity,
    ...
    body:       JSON.stringify({
      ...
      ipsAlertRef:       ipsResult?.alertRef ?? null,
    }),
    sent_at:    new Date().toISOString(),
    email_to:   null,
  });
...
/**
 * Render the operator email body for a host-SSH-audit alert.
 *
 * @param {{ anomalies: Array<object>, acceptedCount: number, failedCount: number,
 *           lookbackMinutes: number }} result
 * @param {string} applianceName
 * @param {string} alertRef
 * @returns {string}
 */
function buildHostSshAuditEmailBody(result, applianceName, alertRef) {
  const lines = [];
  lines.push(`COSA inbound-SSH audit on ${applianceName} flagged anomalous login activity.`);
  lines.push('');
  lines.push(`Ref:        ${alertRef}`);
  lines.push(`Window:     last ${result.lookbackMinutes} min`);
  lines.push(`Logins:     ${result.acceptedCount} accepted, ${result.failedCount} failed`);
  lines.push('');
  lines.push('─── ANOMALIES ────────────────────────────────────────────');
  for (const a of result.anomalies) {
    switch (a.type) {
      case 'untrusted_accepted_login':
        lines.push(`  • [${a.severity}] ACCEPTED login from UNTRUSTED IP ${a.sourceIp} as "${a.user}"`);
        lines.push(`      ${a.sample}`);
        break;
      case 'unexpected_user_login':
        lines.push(`  • [${a.severity}] ACCEPTED login as unexpected user "${a.user}" from ${a.sourceIp}`);
        lines.push(`      ${a.sample}`);
        break;
      case 'failed_login_attempt':
        lines.push(`  • [${a.severity}] ${a.count} failed/probe attempt(s) from ${a.sourceIp}` +
          (a.user ? ` (users: ${a.user})` : ''));
        lines.push(`      ${a.firstSeen} → ${a.lastSeen}`);
        lines.push(`      ${a.sample}`);
        break;
      default:
        lines.push(`  • [${a.severity}] ${JSON.stringify(a)}`);
    }
  }
  lines.push('');
  lines.push('─── CONTEXT ──────────────────────────────────────────────');
  lines.push('  Only the owner laptop should ever log into COSA. Any accepted login');
  lines.push('  from another source, or ANY failed attempt, is unexpected on this host.');
  lines.push('');
  lines.push('─── RESPONSE OPTIONS ─────────────────────────────────────');
  lines.push('  • SSH in and review:  journalctl -t sshd -t sshd-session -o short-iso');
  lines.push('  • If unrecognised: rotate keys, check sshd_config, consider fail2ban');
  lines.push('  • If the laptop IP changed (DHCP): update trusted_ips in appliance.yaml');
  lines.push('');
  lines.push('──────────────────────────────────────────────────────────');
  lines.push('This alert was generated by COSA (Compute-Oriented System Agent).');
  return lines.join('\n');
...
/**
 * Audit COSA's OWN inbound SSH activity (local journald) hourly.
 *
 * Invoked directly (no Claude session — the detection is fully deterministic).
 * On a host that should only ever see logins from one laptop, any untrusted
 * accepted login or any failed attempt is high-signal, so a finding always
 * emails the operator. Groups all anomalies into one email per run and
 * deduplicates by (category, maxSeverity) within HOST_SSH_AUDIT_DEDUP_WINDOW_MS.
 *
 * @returns {Promise<void>}
 */
async function runHostSshAuditTask() {
  const { appliance } = getConfig();
  const cfg = appliance.tools?.host_ssh_audit ?? {};

  // Belt-and-suspenders on top of the cron-skip guard in start().
  if (cfg.enabled === false) return;

  const operatorEmail = appliance.operator?.email;
  const applianceName = appliance.appliance?.name ?? 'COSA';

  let result;
  try {
    result = await hostSshAuditTool.handler();
  } catch (err) {
    log.error(`[host-ssh-audit] tool threw: ${err.message}`);
    return;
  }

  if (result.skipped || !result.anomalies || result.anomalies.length === 0) {
    log.info('[host-ssh-audit] complete: 0 anomalies');
    return;
  }

  log.info(`[host-ssh-audit] complete: ${result.anomalies.length} anomaly(s)`);

  // Highest severity across all anomalies for dedup and the alert record.
  const SEVERITY_ORDER = ['critical', 'high', 'medium', 'low'];
  const severities     = result.anomalies.map(a => a.severity ?? 'low');
  const maxSeverity    = SEVERITY_ORDER.find(s => severities.includes(s)) ?? 'low';

  // Dedup: suppress if the same (category, severity) was alerted recently.
  const sinceIso = new Date(Date.now() - HOST_SSH_AUDIT_DEDUP_WINDOW_MS).toISOString();
  const recent   = findRecentAlert(HOST_SSH_AUDIT_CATEGORY, maxSeverity, sinceIso);
  if (recent) {
    log.info(`[host-ssh-audit] suppressed duplicate ${maxSeverity} alert (last sent: ${recent.sent_at})`);
    return;
  }

  const alertRef = `SSH-${Date.now()}`;
  const title    = `${applianceName} — Inbound SSH Anomaly`;
  const body     = buildHostSshAuditEmailBody(result, applianceName, alertRef);
  const sentAt   = new Date().toISOString();
  ...
  createAlert({
    session_id: null,
    severity:   maxSeverity,
    category:   HOST_SSH_AUDIT_CATEGORY,
    title,
    body,
    sent_at:    sentAt,
    email_to:   operatorEmail,
  ...
  log.info(`[host-ssh-audit] alert sent: ${alertRef} (${maxSeverity}, ${result.anomalies.length} anomalies)`);
}
...
function _taskRunnerMap() {
  return {
    ...
    host_ssh_audit:            runHostSshAuditTask,
...
function start() {
  ...
  // COSA's OWN inbound-SSH audit — hourly at :30 (staggered off the minute-0
  // jobs). Reads the local journal (not the appliance) and emails on any
  // untrusted accepted login or failed attempt.
  schedule('host_ssh_audit', '30 * * * *', runHostSshAuditTask);
